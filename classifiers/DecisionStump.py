    ...
    def fit(self, X, y):
        ...
        for d in range(D):
            for n in range(N):
                ...
                y_sat = common.mode(y[X[:, d] == value])
                y_not = common.mode(y[X[:, d] != value])
...
def entropy(p):
    """
    A helper function that computes the entropy of the discrete
    distribution p (stored in a 1D numpy array).
    :param p: a 1D numpy array whose elements should add up to 1
    :return: entropy of p
    """
    plogp = 0 * p
