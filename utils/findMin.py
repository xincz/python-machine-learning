...
def findMinL1(funObj, w, L1_lambda, maxEvals, *args, verbose=0):
    ...
    The line search algorithm divides the step size by 2 until it find the
    step size that results in a decrease of the L1 regularized objective
    function.
    """
    # Parameters of the Optimization
    optTol = 1e-2
...
def findMinSGD(funObj, w, maxEvals, minibatch_size, learning_rate, epoch, X, y, verbose=0):
    """
    Uses stochastic gradient descent to optimize the objective function.
    This uses quadratic interpolation in its line search to determine
    the step size alpha.
    """
    # Parameters of the Optimization
    optTol = 1e-2
    ...
    n, d = X.shape
    minibatch = np.random.choice(n, size=minibatch_size, replace=False)
    ...
    alpha = learning_rate
    # Equality: n * epoch = num_iters * minibatch_size
    num_iters = n * epoch / minibatch_size
    ...
    while True:
        ...
        if funEvals >= num_iters:
            if verbose:
                print("Reached maximum number of function evaluations %d" % num_iters)
            break
